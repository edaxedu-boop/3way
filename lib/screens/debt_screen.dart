...
class _DebtScreenState extends State<DebtScreen> {
  ...
  late Future<List<Transaction>> _debtTransactions;
  ...
  void _loadTransactions() {
    setState(() {
      _debtTransactions = dbHelper.getTransactions('debt');
    });
  }

  void _addDebt(String title, double amount) async {
    final newTransaction = Transaction(title: title, amount: amount, date: DateTime.now());
    await dbHelper.insertTransaction(newTransaction, 'debt');
    _loadTransactions();
  }

  void _updateDebt(Transaction transaction, String newTitle, double newAmount) async {
  ...
  void _showEditDebtDialog(Transaction transaction) {
            ...
            ElevatedButton(
              onPressed: () {
                ...
                if (newTitle.isNotEmpty && newAmount > 0) {
                  _updateDebt(transaction, newTitle, newAmount);
                  Navigator.of(context).pop();
                }
              },
                    ...
                    trailing: Row(
                      ...
                      children: [
                        ...
                        PopupMenuButton(
                          ...
                          onSelected: (value) {
                            if (value == 'edit') {
                              _showEditDebtDialog(transaction);
