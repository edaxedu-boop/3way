...
class DatabaseHelper {
  ...
  Future<int> addTransaction(Transaction transaction, {DatabaseExecutor? txn}) async {
  ...
  Future<int> _addTransactionLogic(Transaction transaction, DatabaseExecutor txn) async {
    final newId = await txn.insert('transactions', transaction.toMapWithoutId(), conflictAlgorithm: ConflictAlgorithm.replace);
  ...
  Future<void> _deleteTransactionLogic(int id, DatabaseExecutor txn) async {
    final transactions = await txn.query('transactions', where: 'id = ?', whereArgs: [id]);
    ...
    if (transaction.type == 'income') {
      ...
      await txn.rawUpdate('UPDATE budget_summary SET $columnToUpdate = $columnToUpdate + ? WHERE id = 1', [transaction.amount]);
    }
  ...
  Future<Map<String, double>> getBudgetSummary({DatabaseExecutor? txn}) async {
    final db = txn ?? await database;
    final result = await db.query('budget_summary', where: 'id = ?', whereArgs: [1]);
  ...
  Future<void> payDebt(int debtId, double amount, String category) async {
    final db = await database;
    await db.transaction((txn) async {
      final debts = await txn.query('debts', where: 'id = ?', whereArgs: [debtId]);
      ...
      await addTransaction(Transaction(title: 'Abono a deuda: ${debt.title}', amount: amount, date: DateTime.now(), type: 'expense', category: category), txn: txn);
      if (newRemainingAmount <= 0) {
        await txn.delete('debts', where: 'id = ?', whereArgs: [debtId]);
      } else {
        await txn.update('debts', {'remainingAmount': newRemainingAmount}, where: 'id = ?', whereArgs: [debtId]);
      }
    });
  }
  ...
  Future<void> insertInvestment(String title, double amount, DateTime date, String category) async {
    final db = await database;
    await db.transaction((txn) async {
  ...
  Future<List<Investment>> getInvestments() async {
    final db = await database;
    final List<Map<String, dynamic>> maps = await db.query('investments', orderBy: 'date DESC');
  ...
  Future<void> deleteInvestment(int id) async {
    final db = await database;
    await db.transaction((txn) async {
      final investments = await txn.query('investments', where: 'id = ?', whereArgs: [id]);
  ...
  Future<Map<String, double>> getSummary() async {
    ...
    final totalBalance = budgetSummary['needs']! + budgetSummary['wants']! + budgetSummary['savings']!;
    final db = await database;
  ...
  Future<List<Transaction>> getRecentTransactions({int limit = 5}) async {
    final db = await database;
    final List<Map<String, dynamic>> maps = await db.query('transactions', orderBy: 'date DESC', limit: limit);
